...
var dataModel = mongoose.model('case3active', schema);//(文档，schema)定义了一个model
var Q = require('q');

function removeSaveData(docs) {
    var removeDataModel = mongoose.model('case3active', schema);//(文档，schema)定义了一个model
...
function saveData(docs) {
    ...
    params.paramNoDb("case3active", function (result) {
...
///获取grid表头格式
function getGrid(cb) {
    params.paramNoDb("case3activeGrid", function (result) {
        cb(result);
    });
}
...
function getDataForExcel(req, res, cb) {
    ...
    params.paramNoDb("case3activeExcel", function (result) {
        var excelHeader;
        excelHeader = result;
        params.paramNoDb("case3active", function (result) {
