...
function removeSaveData(docs) {
    var removeDataModel = mongoose.model('case5sale', schema);//(文档，schema)定义了一个model
...
function saveData(docs) {
    ...
    params.paramNoDb("case5sales", function (result) {
        SchemaParams = eval("(" + result + ")");
        // console.log('saveData-sales=' + result);
        schema.add(SchemaParams);
        var dataModel = mongoose.model('case5sale', schema);//(文档，schema)定义了一个model
...
///获取grid表头格式
function getGrid(cb) {
    params.paramNoDb("case5salesGrid", function (result) {
        cb(result);
    });
}
...
function getDataForExcel(req, res, cb) {
    ...
    params.paramNoDb("case5salesExcel", function (result) {
        var excelHeader;
        excelHeader = result;
        params.paramNoDb("case5sales", function (result) {
            SchemaParams = eval("(" + result + ")");//貌似查询的时候不用定义schema格式，返回所有字段
