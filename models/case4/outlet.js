/**
 *功能：保存客户资料信息（客户资料.xlsx）到outlet文档中
...
var Q = require('q');
var dataModel = mongoose.model('case4outlet', schema);//(文档，schema)定义了一个model


function removeSaveData(docs) {
    var removeDataModel = mongoose.model('case4outlet', schema);//(文档，schema)定义了一个model
...
function saveData(docs) {
    ...
    params.paramNoDb("case4outlet", function (result) {
...
///获取grid表头格式
function getGrid(cb) {
    params.paramNoDb("case4outletGrid", function (result) {
        cb(result);
    });
}
...
function getDataForExcel(req, res, cb) {
    ...
    params.paramNoDb("case4outletExcel", function (result) {
        var excelHeader;
        excelHeader = result;
        params.paramNoDb("case4outlet", function (result) {
            SchemaParams = eval("(" + result + ")");//貌似查询的时候不用定义schema格式，返回所有字段
