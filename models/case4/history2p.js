...
function getGrid(req, res, cb) {
    ...
    var condition = "";
    var collection = 'case4result2p' + period + 'v' + vno;
    ...
    params.paramNoDb("versionsGrid", function (result1) {//历史清单表头
        dataModel.find({ 'collname': 'header' }, function (err, header) {//历史表头参数
            params.paramNoDbv('case4calcResultGrid2p', 'case4', header, function (result2) {
...
function getDataHistoryForExcel(req, res, cb) {
    ...
    dataModel.find({ 'collname': 'header' }, function (err, caseheader) {
        ...
        params.paramNoDbv('case4calcResultExcel2p', 'case4', caseheader, function (result) {
            var excelHeader;
            excelHeader = result;
...
function getData(req, res, cb) {
    ...
    params.paramNoDb("versions", function (result) {
        ...
        var dataModel = mongoose.model('case4version22p', schema, 'case4versions2ps');//(文档，schema)定义了一个model
