...
function removeSaveData(docs) {
    var removeDataModel = mongoose.model('case5sku22', schema, 'case5skus');//(文档，schema)定义了一个model
...
function saveData(docs) {
    ...
    params.paramNoDb("case5sku", function (result) {
        SchemaParams = eval("(" + result + ")");
        //console.log('saveData=' + result);
        schema.add(SchemaParams);
        var dataModel = mongoose.model('case5sku1', schema, 'case5skus');//(文档，schema)定义了一个model
...
///获取grid表头格式
function getGrid(cb) {
    params.paramNoDb("case5skuGrid", function (result) {
        cb(result);
    });
}
...
function getData(req, res, cb) {
    ...
    params.paramNoDb("case5sku", function (result) {
        ...
        var dataModel = mongoose.model('case5sku2', schema, 'case5skus');//(文档，schema)定义了一个model
...
function getDataForExcel(req, res, cb) {
    ...
    params.paramNoDb("case5skuExcel", function (result) {
        var excelHeader;
        excelHeader = result;
        params.paramNoDb("case5sku", function (result) {
            SchemaParams = eval("(" + result + ")");//貌似查询的时候不用定义schema格式，返回所有字段
            schema.add(SchemaParams);
            var dataModel = mongoose.model('case5sku', schema);//(文档，schema)定义了一个model
            //console.log(condition);
            dataModel.find(condition, function (err, docs) {
