...
function removeSaveData(docs) {
    var removeDataModel = mongoose.model('case5deliver', schema);//(文档，schema)定义了一个model
...
function saveData(docs) {
    ...
    params.paramNoDb("case5deliver", function (result) {
        SchemaParams = eval("(" + result + ")");
        // console.log('saveData-delivers=' + result);
        schema.add(SchemaParams);
        var dataModel = mongoose.model('case5deliver', schema);//(文档，schema)定义了一个model
...
///获取grid表头格式
function getGrid(cb) {
    params.paramNoDb("case5deliverGrid", function (result) {
        cb(result);
    });
}
...
function getDataForExcel(req, res, cb) {
    ...
    params.paramNoDb("case5deliverExcel", function (result) {
        var excelHeader;
        excelHeader = result;
        params.paramNoDb("case5deliver", function (result) {
            SchemaParams = eval("(" + result + ")");//貌似查询的时候不用定义schema格式，返回所有字段
