/**
 *功能：保存售点考核结果（1、售点考核结果.xlsx）到case5checkresult文档中
...
var dataModel = mongoose.model('case5checkresult', schema);//(文档，schema)定义了一个model
var tempColl = require("./tempCollection.js");
var Q = require('q');

function removeSaveData(docs) {
    var removeDataModel = mongoose.model('case5checkresult', schema);//(文档，schema)定义了一个model
...
function saveData(docs) {
    ...
    params.paramNoDb("case5checkresult", function (result) {
...
///获取grid表头格式
function getGrid(cb) {
    params.paramNoDb("case5checkresultGrid", function (result) {
        cb(result);
    });
}
...
function getDataForExcel(req, res, cb) {
    ...
    params.paramNoDb("case5checkresultExcel", function (result) {
        var excelHeader;
        excelHeader = result;
        params.paramNoDb("case5checkresult", function (result) {
            SchemaParams = eval("(" + result + ")");//貌似查询的时候不用定义schema格式，返回所有字段
