...
var Q = require('q');
var dataModel = mongoose.model('case5outlet', schema);//(文档，schema)定义了一个model


function removeSaveData(docs) {
    var removeDataModel = mongoose.model('case5outlet', schema);//(文档，schema)定义了一个model
...
///获取grid表头格式
function getGrid(cb) {
    params.paramNoDb("case5outletGrid", function (result) {
        cb(result);
    });
}
...
function getDataForExcel(req, res, cb) {
    ...
    params.paramNoDb("case5outletExcel", function (result) {
        var excelHeader;
        excelHeader = result;
        params.paramNoDb("case5outlet", function (result) {
            SchemaParams = eval("(" + result + ")");//貌似查询的时候不用定义schema格式，返回所有字段
